// app/(dashboard)/movimentacoes/page.tsx
'use client'

import { useState, useEffect, useCallback } from 'react'
import { Plus, Search, ArrowLeftRight, ChevronLeft, ChevronRight, Loader2 } from 'lucide-react'
...
export default function MovimentacoesPage() {
  ...
  const [form, setForm] = useState({ productId: '', warehouseId: '', tipo: 'ENTRADA', quantidade: '', observacao: '' })
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const params = new URLSearchParams({ q, tipo, page: String(page), limit: String(limit) })
  ...
  async function openSheet() {
    ...
    setForm({ productId: '', warehouseId: wh?.id ?? '', tipo: 'ENTRADA', quantidade: '', observacao: '' })
  }
    ...
    <div className="p-5 flex flex-col gap-4 h-full overflow-auto">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-sm font-semibold text-foreground">Movimentações de Estoque</h1>
      ...
      <div className="flex gap-2">
        ...
        <select value={tipo} onChange={e => { setTipo(e.target.value); setPage(1) }}
          className="h-8 px-2.5 rounded-lg border border-border bg-card text-[12px] focus:outline-none focus:border-ring cursor-pointer">
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-border">
              ...
              <th className="px-3 py-2 text-left text-[10px] uppercase tracking-widest font-semibold text-muted-foreground">Antes→Depois</th>
              ...
              <th className="px-3 py-2 text-left text-[10px] uppercase tracking-widest font-semibold text-muted-foreground">Obs</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={8} className="px-3 py-12 text-center">
                  <div className="flex flex-col items-center gap-2 text-muted-foreground">
                    ...
                    <span className="text-[12px]">Nenhuma movimentação encontrada</span>
                  </div>
                </td>
              </tr>
            ) : items.map(item => (
              <tr key={item.id} className="border-t border-border hover:bg-surface-elevated transition-colors">
                ...
                <td className="px-3 py-2 text-[12px] text-muted-foreground">{item.estoqueAntes} → {item.estoqueDepois}</td>
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent className="w-[420px]">
          <SheetHeader>
            <SheetTitle className="text-sm font-semibold">Nova Movimentação</SheetTitle>
          </SheetHeader>
